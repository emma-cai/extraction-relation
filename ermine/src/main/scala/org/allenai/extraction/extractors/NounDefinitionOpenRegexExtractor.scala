...
class NounDefinitionOpenRegexExtractor(dataPath: String) extends DefinitionOpenRegexExtractor(dataPath,  "noun") {
  ...
  override def process(allTypes: Seq[Type], lastLevelTypes: Seq[Type]): Seq[String] = {
    var results: Seq[String] = Seq.empty[String]
    
    // Make a Type name -> Type map out of the output Types for easy lookup
    val lastLevelTypeTuples: Seq[Tuple2[String, Type]] = lastLevelTypes.map(typ => (typ.name, typ))

    // Iterate over all the returned Types from the topmost level in the cascade and process
    // each Type that is in the above list.
    for ((lastLevelTypeName, lastLevelType) <- lastLevelTypeTuples) {
      if ((results.length == 0) && (definitionTypeNames.contains(lastLevelTypeName)))
        results = processNounDefinition(lastLevelType, allTypes)
    }
    results
  }
  ...
  private def processNounDefinitionIsa(typ: Type, types: Seq[Type]): (Option[String], Seq[String]) = {
    ...
    (definedTermOption, defnIsaOption) match {
         case (Some(definedTerm), Some(defnIsa)) => {
           val definedTerm: String = definedTermOption.get
           val defnIsa = defnIsaOption.get
  ...
  private def nounDefinitionGetDefinedTerm(isaOption: Option[NamedGroupType], types: Seq[Type]): (Option[String], Option[Type]) = {
    var definedTermOption: Option[String] = None
    var defnIsaOption: Option[Type] = None
    definedTermOption = isaOption match {
      case Some(isa) =>
  ...
  def getFormattedVP1(typ: Type, types: Seq[Type]): String = {
    var result: String = ""
    val rel = Extractor.findSubtypesWithName(types)(typ, "Rel").headOption match {
      case Some(rl) => rl.text
      case _ => ""
    }
    val arg2 = Extractor.findSubtypesWithName(types)(typ, "Arg2").headOption match {
      case Some(a) => a.text
      case _ => ""
    }
    val arg = Extractor.findSubtypesWithName(types)(typ, "Arg").headOption match {
      case Some(a) => a.text
      case _ => ""
    }
    if (rel.length() > 0) {
  ...
  private def getFormattedS(typ: Type, types: Seq[Type]): String = {
    ...
    val antecedentS: String =
      Extractor.findSubtypesWithName(types)(typ, "AntecedentS").headOption match {
        case Some(antecedent) =>
          Extractor.findAlignedTypesWithName(types)(antecedent, "S1").headOption match {
            case Some(s1) => getFormattedS1(s1, types)
            case _ => ""
          }
        case _ => ""
    ...
    if ((antecedentS.length() > 0) && (rel.length() > 0) && (consequentS.length() > 0)) {
      result += "(" + antecedentS + ", " + rel + ", " + consequentS + ")"
    }
    result
  }
