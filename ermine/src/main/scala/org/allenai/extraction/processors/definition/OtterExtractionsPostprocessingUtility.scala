...
object OtterExtractionsPostprocessingUtility {
  ...
  def processExtraction(extraction: OtterExtractionTuple) : Option[OtterExtractionTuple] = {          
    extraction  match {
      ...
      case SimpleOtterExtractionTuple(_,_,_, relation, Some(relObj), advps, pps) 
        if (relation.relationType.isDefined && (relation.relationType.get == RelationTypeEnum.IsA)
      ...
      case SimpleOtterExtractionTuple(tokens, interval, agent, relation, Some(relObj), advps, pps) 
        if (relation.relationType.isDefined && (relation.relationType.get == RelationTypeEnum.IsA)
