  ...
  class InternalProcessor(source: Source, focus: String, destination: Writer) {
    ...
    def process(): Unit = {
      ...
      for (word <- focus.split(' ')) {
        ...
        for (map <- DependencyGraph.executeSparql(graph, tokenQuery)) {
          tokens += map("token")
        }
      }
      if (tokens.isEmpty) {
      ...
      ruleId += 1
      destination.write(s"rule${ruleId}:: ")
      destination.write(rule.toString)
      destination.write(".\n")
    ...
    def parentNode(tokens: Array[Vertex]): Option[Vertex] = {
      ...
      while (parent.isDefined) {
        val parentConstits: Seq[Vertex] = DependencyGraph.nodeConstits(graph, parent.get) :+ parent.get
    ...
    def nodeParent(node: Vertex): Option[Vertex] = {
      ...
      val results: Seq[Map[String, Vertex]] = DependencyGraph.executeSparql(graph, nodeQuery)
