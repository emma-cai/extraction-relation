...
top_relation(Root) :-
	findall(Root,
		(relation(Root,Json), json:json_write(user_output,Json), nl),
		Roots),
	Roots \= [], !.
...
relation(Root,Json) :-
	...
	tuple(Root,Tuple),
	write_relation(Entity,Rel,Tuple,Json).
% purpose (tuple-NP)
relation(Root,Json) :-
	...
	write_relation(Tuple,Rel,Entity,Json).
% effect (tuple-tuple)
relation(Root,Json) :-
	...
	write_relation(Tuple1Out,Rel,Tuple2Out,Json).
% function (NP-tuple)
relation(Root,Json) :-
	...
	write_relation(Root,Rel,TupleOut,Json).
% function (NP-NP)
relation(Root,Json) :-
	function(Root,Comp,Rel),
	dependency(_,dep:prep_for,Comp), % responsible for
	write_relation(Root,Rel,Comp,Json).
% example (NP-tuple)
relation(Root,Json) :-
	example_NP_Tuple(Root,Comp,Rel),
	tuple(Comp,[Subj|Rest]),
	Subj \= Root, % from rcmod
	write_entity_relation(Root,Rel,[Subj|Rest],Json).
% example (tuple-NP)
relation(Root,Json) :-
	example_Tuple_NP(Root,Comp,Rel),
	tuple(Comp,Tuple),
	write_entity_relation(Tuple,Rel,Root,Json).
% example (NP-NP)
relation(Root,Json) :-
	example_NP_NP(Root,Entity,Rel),
	write_entity_relation(Root,Rel,Entity,Json).
